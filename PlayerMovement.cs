...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] float shotReloadTime; // 0.1f; //determines the spawn rate of the bullets when you hold down the shoot key
    ...
    public State PlayerState
    {
        ...
        {
            playerState = value;
            //StopAllCoroutines(); //It would be cleaner to just stop all the coroutines here and restart the one we need, but the problem is that StopAllCoroutines won't take effect instantly,
            //so it would override the effect of any StartCoroutines we perform here. More notes about this in the HomingAlien class, which has a similar structure.
            switch (value)
            {
    ...
    [HideInInspector] State playerState = State.Uncontrollable; //Uncontrollable
    ...
    IEnumerator ControllingPlayer()
    {
        ...
        while (playerState == State.Controllable)
        {
            ...
            //POSSIBLE TODO: Platform games usually implement a "late jump leeway", which allows the player to jump even if he's left a platform already, e.g. jump can be performed even
